        ...
        for(int i = 0; i < nNode; i++) {
            for (int j = i+1; j < nNode; j++) {
                ...
                if (nodes[j] instanceof Order)
                    ethaMatrix[i][j] += Q / (Duration.between(((Order)nodes[j]).twOpen, ((Order)nodes[j]).twClose).toHours()/4.0) + Q * (((Order) nodes[j]).totalDemand/10);
                else ethaMatrix[i][j] += 0.75;
                ethaMatrix[j][i] = Q / (distanceMatrix[j][i]+1);
                if (nodes[i] instanceof Order)
                    ethaMatrix[j][i] += Q / (Duration.between(((Order)nodes[i]).twOpen, ((Order)nodes[i]).twClose).toHours()/4.0) + Q * (((Order) nodes[i]).totalDemand/10);
